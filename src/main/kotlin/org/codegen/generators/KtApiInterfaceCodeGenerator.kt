...
class KtApiInterfaceCodeGenerator: AbstractCodeGenerator() {
    ...
    private fun buildInterfaceExtensions(entity: Entity): String =
        entity.methods.joinToString(separator = "\n") { method ->
    ...
    override fun build(): String {
        ...
        val builtInterfaces = getEntities().map { interfaceGen.buildEntity(it) }
        val buildRequests = getEntities(includeMethods = true).map { requestGen.buildEntity(it) }
        val buildExtensions = getEntities().map { buildInterfaceExtensions(it) }
