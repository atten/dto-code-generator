...
Console tool for generating API clients, data classes and validators. Written in Kotlin, produces Kotlin and Python code. For the input it takes OpenApi document (JSON/YAML, v2.0 ‒ v3.2.0) or custom codegen schema (JSON).
...
```shell
wget -qO- https://github.com/atten/ez-code-generator/releases/download/v3.2.0/ez-codegen-3.2.0.zip | busybox unzip -
cd ez-codegen-3.2.0/bin/
chmod +x ez-codegen
```
...
```shell
pip install ez-code-generator==3.2.0
```
